...
def baseline_model(cur_row: pd.Series):
    ...
    results_dict = {
        ...
        'message': None, 'mTEMisp': None, 'Tcm': None, 'Thm': None, 'Pelem_arr': None,
    ...
    if calccontin == 1:
        ...
        if calccontin == 1:
            ...
            if calccontin == 1:
                ir, irw, nTEMrowcur, Lpolez1 = 1, 0, nTEMrowhm, 1
                # количество вариантов числа ТЭМ в ряду
                while irw == 0:
                    if Lpolez1 > Lpolez:
                        nTEMrowcur -= 2
                        if nTEMrowcur >= 2:
                            ir += 1
                            Lpolez1 = nTEMrowcur/nTEMrowhm
                        else:
                            irw = 1
                    else:
                        irw = 1
                # геометрия допускает изменение конфигурации
                if ir > 1:
                    ...
                    if calccontin == 1:
                        nTEMrowcur = nTEMrowhm - 2
                        TEMkonf, rowcur, rowpred, nTEMrowpred, rowch = 0, 1, 0, nTEMrowhm, 0
                        mTcmout1 = mTcmout.copy()
                        mTcmout3 = mTcmout.copy()
                        PTEG3, rtcGsv3, rownumtot3, nTEMrowcur3, rowcur3, nTEMrowpred3, rowpred3 = PTEG, rtchm, rownumtot, nTEMrowhm, 1, 0, 0
                        nTEMrowrezm = np.full(rownumtot + 1, nTEMrowhm)
                        nTEMrowrezm[1] = nTEMrowhm - 2
                        nTEMrowrezm3 = np.full(rownumtot + 1, nTEMrowhm)
                        nTEMprkr = nTEMrowhm - 1
                        mTEMisp = np.full((rownumtot+1, nTEMrowhm+1), 1)
                        mTEMisp[1, 2], mTEMisp[1, nTEMprkr] = 0, 0
                        mTEMisp3 = np.full((rownumtot+1 , nTEMrowhm+1), 1)

                        rez, rez1, Pmax, rowstop = 0, 0, 0, 0
                        # подбор конфигурации
                        while TEMkonf == 0:
                            ...
                            while teplsuc == 0:
                                    ...
                                    if (Thmrest < delThm or rtcGrest < rtcGrow) or (
                                        ...
                                        if Thmrest < delThm or rtcGrest < rtcGrow:
                                            if rownum == rownumtot1: # завершение итераций теплового расчёта
                                                ...
                                                rownumtot1 -= 1
                                                rowf = 1
                                                # последний ряд сравнялся с конфигурируемым
                                                if rownumtot1 == rowcur:
                                                    teplsuc, rowstop, TEMkonf = 1, 1, 1
                                                    PTEG1 = PTEG3;
                                                    rtcGsv = rtcGsv3
                                                    rowcur = rowcur3
                                                    rowpred = rowpred3
                                                    rownumtot1 = rownumtot3
                                                    nTEMrowpred = nTEMrowpred3
                                                    nTEMrowcur = nTEMrowcur3
                                                    mTcmout = mTcmout3.copy()
                                                    nTEMrowrezm = nTEMrowrezm3.copy()
                                                    mTEMisp = mTEMisp3.copy()
                                        # есть запас по температуре и сопротивлению
                                        if Thmrest >= delThm and rtcGrest >= rtcGrow:
                                            if rownum == rowmax: # превышено число рядов
                                                teplsuc, rowstop, TEMkonf, calccontin = 1, 1, 1, 0
                                            else: # уточнение числа рядов
                                                if rowf == 1:
                                                    teplsuc = 1
                                                else:
                                                    nTEMrowrezm = nTEMrowrezm[:rownumtot1 + 1].copy()
                                                    mTEMisp = mTEMisp[:rownumtot1+1, :].copy()
                                                    rownumtot1 += 1
                                                    nTEMrowrezm = np.append(nTEMrowrezm, nTEMrowhm)
                                                    mTEMisp = np.append(
                                                        mTEMisp,
                                                        np.ones((1, mTEMisp.shape[1]), dtype=mTEMisp.dtype),
                                                        axis=0
                                                    )
                                ...
                                if teplsuc == 0:
                                    ...
                                    # параметры газа в последующих рядах
                                        ...
                                        # нет пустых слотов в предыдущем ряду
                                        if svin == nTEMrowhm:
                                            if TEMnum == TEMnum1:
                                                whmin = (mwhmout[imas-TEMnum1] + mwhmout[imas-TEMnum1+1] + mwhmout[imas-TEMnum1+2])/3
                                                Thmin = (mThmout[imas-TEMnum1] + mThmout[imas-TEMnum1+1] + mThmout[imas-TEMnum1+2])/3
                                            if TEMnum < TEMnum1 and TEMnum > 1:
                                                whmin = (mwhmout[imas-TEMnum1-1] + mwhmout[imas-TEMnum1] + mwhmout[imas-TEMnum1+1])/3
                                                Thmin = (mThmout[imas-TEMnum1-1] + mThmout[imas-TEMnum1] + mThmout[imas-TEMnum1+1])/3
                                            if TEMnum == 1:
                                                whmin = (mwhmout[imas-TEMnum1-2] + mwhmout[imas-TEMnum1-1] + mwhmout[imas-TEMnum1])/3
                                                Thmin = (mThmout[imas-TEMnum1-2] + mThmout[imas-TEMnum1-1] + mThmout[imas-TEMnum1])/3
                                        # для свиньи
                                        else:
                            ...
                            if calccontin == 1:
                                delPTEG = abs(PTEG3 - PTEG1)*100/PTEG3
                                Thmrest = 0
                                masparam = nTEMrowrezm[rownumtot1]
                                for jm in range(1, masparam+1):
                                    Thmrest += mThmout[imas - jm]
                                Thmrest = Thmrest/masparam - Thmpr
                                if PTEG1 > PTEG3 and Thmrest >= 0: # конфигурация обеспечила прирост мощности
                                    Pmax = 0
                                    PTEG3 = PTEG1
                                    rtcGsv3 = rtcGsv
                                    rowcur3 = rowcur
                                    rowpred3 = rowpred
                                    rownumtot3 = rownumtot1
                                    nTEMrowpred3 = nTEMrowpred
                                    nTEMrowcur3 = nTEMrowcur
                                    # сохранение конфигурации слоя
                                    mTcmout3 = mTcmout.copy()
                                    nTEMrowrezm3 = nTEMrowrezm.copy()
                                    mTEMisp3 = mTEMisp.copy()
                                else:
                                    Pmax = 1

                            # текущий ряд не больше предпоследнего - адаптивное конфигурирование
                            if rowstop == 0:
                                # для первого ряда
                                if rowcur == 1:
                                    if Pmax == 0:
                                        rez = 1
                                    # обеспечен прирост мощности, либо снижени мощности и переохлаждение несущественны
                                    if (Pmax == 0) or (
                                        Pmax == 1 and rez == 0 and delPTEG <= 15 and Thmrest >= -50
                                    ):
                                        Lpolez1 = (nTEMrowcur - 2)/nTEMrowhm
                                        if Lpolez1 >= Lpolez:
                                            nTEMrowcur -= 2
                                            nTEMrowrezm[1] = nTEMrowcur
                                        else:
                                            rowch = 1
                                    # прочие негативные результаты
                                    else:
                                        # прирост мощности был обеспечен для предшествовавшей конфигурации
                                        if rez == 1:
                                            rowch = 1
                                        # прирост мощности не обеспечивался и произошло существенное снижение мощности или переохлаждение
                                        else:
                                            TEMkonf = 1
                                            PTEG1 = PTEG3
                                            rtcGsv = rtcGsv3
                                ...
                                if rowch == 1:
                                    if Pmax == 1 and rez == 1:
                                        nTEMrowrezm[rowcur] = nTEMrowrezm3[rowcur]
                                        nTEMrowrezm[rowpred] = nTEMrowrezm3[rowpred]
                                        PTEG1 = PTEG3
                                        nTEMrowpred = nTEMrowpred3
                                        nTEMrowcur = nTEMrowcur3
                                        mTEMisp[rowcur] = mTEMisp3[rowcur].copy()
                                        mTEMisp[rowpred] = mTEMisp3[rowpred].copy()
                                    direct1, direct2, rez, rez1 = 1, 0, 0, 0
                                    rowcur += 1
                                    rowpred += 1
                                    nTEMrowpred = nTEMrowcur
                                    nTEMrowfix = nTEMrowpred
                                    nTEMrowcur = nTEMrowhm - 2
                                    nTEMrowrezm[rowcur] = nTEMrowcur
                                    if nTEMrowpred > nTEMrowcur or rowcur == rownumtot1:
                                        TEMkonf = 1
                                        PTEG1 = PTEG3
                                        rtcGsv = rtcGsv3
                                        rowcur = rowcur3
                                        ...
                                        nTEMrowrezm = nTEMrowrezm3.copy()
                                        mTEMisp = mTEMisp3.copy()
                                    rowch = 1 if nTEMrowcur == nTEMrowpred else 0

                                # для последующих рядов
                                else:
                                    if rowcur > 1:
                                        # увеличение числа ТЭМ в предыдущем ряду
                                        if direct1 == 1:
                                            if Pmax == 0:
                                                rez = 1
                                                rez1 = 1
                                            # обеспечен прирост мощности, либо при текущем увеличении количества ТЭМ в ряду вообще не обеспечивался
                                            if (Pmax == 0) or (Pmax == 1 and rez1 == 0):
                                                nTEMrowpred += 2
                                                nTEMrowrezm[rowpred] = nTEMrowpred
                                                if nTEMrowpred == nTEMrowcur:
                                                    direct1, direct2, direct2init = 0, 1, 0
                                            else:
                                                direct1, direct2, direct2init = 0, 1, 1

                                        # уменьшение числа ТЭМ в текущем ряду
                                        if direct2 == 1:
                                            # уменьшение числа ТЭМ в текущем ряду на текущей итерации
                                            if direct2init == 1:
                                                rez1, direct1, direct2 = 0, 1, 0
                                                nTEMrowcur -= 2
                                                nTEMrowrezm[rowcur] = nTEMrowcur
                                                nTEMrowpred = nTEMrowfix
                                                nTEMrowrezm[rowpred] = nTEMrowpred
                                                if nTEMrowcur == nTEMrowfix:
                                                    if (Pmax == 1) and (rez == 0) and (delPTEG > 15 or Thmrest < -50):
                                                        TEMkonf = 1
                                                        PTEG1 = PTEG3
                                                        rtcGsv = rtcGsv3
                                                        rowcur = rowcur3
                                            ...
                                            # нет ТЭМ
                                                ...
                                                # по меньшему промежутку
                                                    ...
                                                    if lzaz1 == 0:
                                                        ...
                                                        sl = 1
                                                        xzaz2 -= 1
                                                if xzaz2 == 0 and yzaz2 == 0 and xzaz3 == 0 and yzaz3 == 0:
                                                    sim = 1
                                        ...
                                        # начиная с центральных слотов
                                            ...
                                            if sim1 == 0:
                                                ...
                                                if xzaz3 == 0 and yzaz3 == 0:
                                                    sim1 = 1
                                                    mTEMisp[rowcur, jm] = 1 #mTEMisp[rowcur, jm-2]

                                            # за центром
                                            else:
                                                mTEMisp[rowcur, nTEMrowhm-s3] = mTEMisp[rowcur, s3+1]
                                                s3 += 1

                                    # заполнение слотов предыдущего ряда
                                    if rowpred > 0:
                                        pustsl = nTEMrowhm - nTEMrowpred
                                        nzaz = nTEMrowpred - 1;
                                        lzazch = pustsl/nzaz - math.trunc(pustsl/nzaz)
                                        lzaz2 = math.trunc(pustsl/nzaz) if lzazch == 0 else math.trunc(pustsl/nzaz) + 1 # длина большого промежутка
                                        lzaz1 = lzaz2 - 1 # длина малого промежутка
                                        ...
                                        for jm in range(1, nTEMrowhm+1):
                                            ...
                                            # начиная с центральных слотов
                                                ...
                                                if sim1 == 0:
                                                    ...
                                                    if xzaz3 == 0 and yzaz3 == 0:
                                                        sim1 = 1
                                                        mTEMisp[rowpred, jm] = 1 #mTEMisp[rowpred, jm-2]

                                                # за центром
                                                else:
                                                    mTEMisp[rowpred, nTEMrowhm-s3] = mTEMisp[rowpred, s3+1]
                                                    s3 += 1
                        ...
                        if calccontin == 1:
                            ...
                            masparam = nTEMrowrezm[1]
                            # конфигурирование не дало результата
                            if masparam == nTEMrowhm:
                                mTcmout = mTcmout1.copy()
                ...
                # нет пустых ячеек
                    ...
                    rownumtot1 = rownumtot
                    nTEMrowrezm = np.full(rownumtot1 + 1, nTEMrowhm)
                    mTEMisp = np.full((rownumtot1 + 1, nTEMrowhm + 1), 1)
                                ...
                                if teplsuc1 == 0:
                                    ...
                                    if slotf == 1:
                                        ...
                                        # параметры горячей среды в последующих рядах
                                            ...
                                            # равенство числа ТЭМ в рассчётном и предыдущем рядах
                                            if svin == nTEMrowhm:
                                                if imas1 == 1:
                                                    whmin = (
                                                        mwhmout[imas - TEMnum1] + mwhmout[imas - TEMnum1 + 1] + mwhmout[imas - TEMnum1 + 2]
                                                    )/3
                                                    Thmin = (
                                                        mThmout[imas - TEMnum1] + mThmout[imas - TEMnum1 + 1] + mThmout[imas - TEMnum1 + 2]
                                                    )/3
                                                if imas1 > 1 and imas1 < TEMnum1:
                                                    whmin = (
                                                        mwhmout[imas - TEMnum1 - 1] + mwhmout[imas - TEMnum1] + mwhmout[imas - TEMnum1 + 1]
                                                    )/3
                                                    Thmin = (
                                                        mThmout[imas - TEMnum1 - 1] + mThmout[imas - TEMnum1] + mThmout[imas - TEMnum1 + 1]
                                                    )/3
                                                if imas1 == TEMnum1:
                                                    whmin = (
                                                        mwhmout[imas - TEMnum1 - 2] + mwhmout[imas - TEMnum1 - 1] + mwhmout[imas - TEMnum1]
                                                    )/3
                                                    Thmin = (
                                                        mThmout[imas - TEMnum1 - 2] + mThmout[imas - TEMnum1 - 1] + mThmout[imas - TEMnum1]
                                                    )/3
                                            # для свиньи
                                            else:
                                                TEMqua, slot1, slot2, chon, slotnum = 0, 0, 0, 0, 0
